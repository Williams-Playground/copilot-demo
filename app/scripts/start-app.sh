...
if ! command -v npm >/dev/null 2>&1; then
    echo "Error: npm was not found in PATH. Install Node.js and npm before starting the Astro client."
    cleanup_processes "$SERVER_PID" "" "$INITIAL_DIR"
    exit 1
fi
...
if ! kill -0 "$SERVER_PID" 2>/dev/null; then
    echo "Error: Flask server exited before startup completed. Check whether port 5100 is already in use."
    cleanup_processes "$SERVER_PID" "$CLIENT_PID" "$INITIAL_DIR"
    exit 1
fi

if ! kill -0 "$CLIENT_PID" 2>/dev/null; then
    echo "Error: Astro client exited before startup completed. Check the npm output above for details."
    cleanup_processes "$SERVER_PID" "$CLIENT_PID" "$INITIAL_DIR"
    exit 1
fi
