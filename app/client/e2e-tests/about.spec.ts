...
test.describe('About Page', () => {
  test('should load about page and display content', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/About - Williams-Playground/);
    
    // Check that the main heading is visible
    await expect(page.getByRole('heading', { name: 'About Williams-Playground' })).toBeVisible();
    ...
    await expect(page.getByText('Williams-Playground is a fictional organization')).toBeVisible();
  });
  ...
  test('should navigate back to homepage from about page', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Welcome to Williams-Playground' })).toBeVisible();
  });
});